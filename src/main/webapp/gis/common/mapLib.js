var lvwMapLib = {
  ...
  initMap: function(mapDiv){
    ...
    for(var i = 0; i < userLayers.length; i++){
      ...
      if(userLayer.type === "wms"){
        var wmsLayer = this.createWmsLayer(userLayer.url, userLayer.subLayers, userLayer.name, userLayer.visible);
  ...
  //定位到点
  zoomTo: function(map, x, y, zoomLevel){
  ...
  //显示弹窗
  showPopup: function(template, map, title, content, width, height, coordinate){
    var popupEle = document.getElementById("popup");
    if(!popupEle){
      $("#overlay").append("<div id=\"popup\" class=\"popup\" style=\"z-index:2002;display:none\"></div>");
      popupEle = document.getElementById("popup");
    }
    popupEle.style.display = "block";
    popupEle.style.width = width + "px";
    popupEle.style.height = height + "px";
    popupEle.innerHTML = template;
    ...
    var titleEle = document.getElementById("popup-title");
  ...
  //创建wms图层
  createWmsLayer: function(url, subLayers, layername, visible){
  ...
  switchTdtMap: function(map, type){
    ...
    $(".toolbar a").removeClass("map_dx_bg");
  ...
  createInFilter: function(fieldName, fieldValues){
    ...
    if(fieldValues.length === 1){
      return queryEqual(fieldName, fieldValues[0].toString());
    }
    ...
    for(var i in fieldValues){
      ...
      var str = "";
      str += "<ogc:PropertyIsEqualTo xmlns:ogc=\"http://www.opengis.net/ogc\">";
      str += "<ogc:PropertyName>" + fieldName + "</ogc:PropertyName>";
      str += "<ogc:Literal>" + fieldValues[i].toString() + "</ogc:Literal>";
