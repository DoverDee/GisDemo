...
function addPointByDB(ctlPrefix, data){
  ...
  for(var i in data){
    if(data.hasOwnProperty(i)){
      var f = new ol.Feature(lvwMapLib.createPoint(data[i].LTTD, data[i].LTTD));
      f.setProperties({
        NAME: data[i].STNM,
        CD: data[i].STCD
      });
...
function addOverlayWithOrder(ctlPrefix, data){
  ...
  for(var i in data){
    if(data.hasOwnProperty(i)){
      ...
      overLayerHtml += "<span class=\"dialMsg\" onclick=\"showDialogForMap('" + ctlPrefix + "','" + oli.STCD + "')\" >" + _index + "</span>";
      overLayerHtml += "</div>";
      // $("#overlay").append("<div id=" + eleId + ">"+data[i].PUMP_NM+"</div>");
      $("#overlaySpecial").html(overLayerHtml);
      var c = [oli.LGTD, oli.LTTD];
